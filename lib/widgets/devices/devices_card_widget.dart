...
class _DevicesCardWidgetState extends State<DevicesCardWidget> {
  ...
  void _updateRelay(String deviceId, String relayId, bool state) async {
    ...
    _fetchData();  // Refresh data after updating relay state
  }
    ...
    return Card(
      ...
      child: Column(
        children: [
          ...
          if (_isExpanded)
            FutureBuilder<List<Map<String, dynamic>>>(  // Fetching devices data
              future: _dataFuture,
              builder: (context, snapshot) {
                if (snapshot.connectionState == ConnectionState.waiting) {
                  return Padding(padding: const EdgeInsets.all(8.0), child: CircularProgressIndicator());
                        ...
                        children: [
                          ...
                          Expanded(
                            child: Column(
                              ...
                              children: [
                                Text(device['name'], style: TextStyle(fontSize: 16, fontWeight: FontWeight.bold)),
                                // Here we'll display the relays and allow updates with Radio buttons
                                if (device['relays'] != null)
                                  ...device['relays'].entries.map((relay) {
                                    return Row(
                                      children: [
                                        Text(relay.key, style: TextStyle(fontSize: 14)),  // Relay name (relay1, relay2, etc.)
                                        Radio<bool>(
                                          value: relay.value,  // Current relay state (true or false)
                                          groupValue: null,  // No grouping, as each relay is separate
